...
什么是物理分表？
**物理分表**：把大表拆成 **多个独立表**，应用层或中间件决定查询哪个表，适合 **超大数据量** 和 **分布式存储**。
**分区表**：MySQL 自动管理多个 **分区**，对 **中等数据量（千万级）** 适用，但**跨分区查询可能影响性能**。
**为什么物理分表比分区表更适合大数据？**
1. **避免跨分区查询**
• **分区表** 在 MySQL 处理 **跨分区查询** 时，可能需要扫描多个分区，导致性能下降。
• **物理分表** 直接根据业务逻辑决定查询哪个表，查询效率更高。
2. **支持分布式存储**
• **分区表** 仍然受限于 MySQL **单机性能**，无法水平扩展到多个数据库实例。
• **物理分表** 可以结合 **分库分表策略**，不同表可以放到不同数据库，提高**并行处理能力**。
3. **索引管理更灵活**
• **分区表** 共享一个索引，部分索引优化受限。
• **物理分表** 每个子表可以独立建索引，提高查询效率。
...
### [禁止给表中的每一列都建立单独的索引](#禁止给表中的每一列都建立单独的索引)
...
### [每个 InnoDB 表必须有个主键](#每个-innodb-表必须有个主键)
...
### [避免建立冗余索引和重复索引（增加了查询优化器生成执行计划的时间）](#避免建立冗余索引和重复索引-增加了查询优化器生成执行计划的时间)

- 重复索引示例：primary key(id)、index(id)、unique index(id)
- 冗余索引示例：index(a,b,c)、index(a,b)、index(a)

### [对于频繁的查询优先考虑使用覆盖索引](#对于频繁的查询优先考虑使用覆盖索引)
...
这段话的意思是：
1. **普通程序使用的数据库账号** **不应该被授予 SUPER 权限**，因为这个权限过高，可能会影响数据库的稳定性和安全性。
2. **MySQL 允许的最大连接数**（由 max_connections 参数控制）达到上限时，普通用户将无法再建立新连接。
3. 但是，**MySQL 仍然会预留 1 个连接给 SUPER 权限的用户**（由 super_read_only 保护的管理员账户），即使数据库连接数已满，DBA 仍可以使用这个 **超级连接** 登录进行维护。
4. **只有 DBA（数据库管理员）应该拥有 SUPER 权限**，以便在数据库出现异常时进行管理和故障处理，比如**杀掉占用连接的进程**、调整配置、恢复服务等。

**正确的做法**
1. **普通程序账号** 只分配**必要的权限**（如 SELECT, INSERT, UPDATE, DELETE 等），**禁止 SUPER 权限**。
2. **DBA 账号**（管理数据库的运维人员）才应该拥有 SUPER 权限，以便在数据库遇到问题时进行维护。
3. **设置合理的最大连接数**（如 max_connections = 200），避免资源被耗尽。
4. **使用 SHOW PROCESSLIST 监控连接情况**，定期优化长连接和无效连接。
